...
static const int scMaximumValue = 1000001;
...
private:
    ...
    public:
        FHexagonalNumbers()
        {
            ...
            PreCalculate(scMaximumValue);
        }

        int GetHexagonalNumber(int n)
        {
            if (Numbers.size() < n)
            {
                CreateHexagonalNumber(n);
            }
            return Numbers[n - 1];
        }

        int GetSize() { return Numbers.size(); }

    private:
        void PreCalculate(int MaximumValue)
        {
            while(Numbers[Numbers.size() - 1] < MaximumValue)
            {
                GetHexagonalNumber(Numbers.size() + 1);
            }
        }

        void CreateHexagonalNumber(int n)
        {
            int BeforeDotCount = GetHexagonalNumber(n - 1);
            int GeneratedDotCount = n + 3 * (n - 1);

            Numbers.push_back(BeforeDotCount + GeneratedDotCount);
        }
    ...
    FHexagonalNumbers HexagonalNumbers;
...
{ 
    DP = std::make_unique<int[]>(scMaximumValue);

    int MaximumHexagonalNumber = HexagonalNumbers.GetSize();
    for(int i = 1; i < MaximumHexagonalNumber; ++i)
    {
        DP.get()[HexagonalNumbers.GetHexagonalNumber(i)] = 1;
    }

    for(int i = 1; i < MaximumHexagonalNumber; ++i)
    {
        int a = HexagonalNumbers.GetHexagonalNumber(i);
        for(int j = 1; j < MaximumHexagonalNumber; ++j)
        {
            int b = HexagonalNumbers.GetHexagonalNumber(j);
            if(a + b <= scMaximumValue)
            {
                DP.get()[a + b] = 2;
            }
        }
...
int FCalculator::Calculate(int N)
{
    return DP.get()[N] == 0 ? 3 : DP.get()[N];
}
...
int Solve()
{
    ...
    std::cout << Calculator.Calculate(N);
